...
class Gradient
{
	...
	public function CoonsPatchMesh($x, $y, $w, $h, $patch_array = [], $x_min = 0, $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
	{
		...
		if ($trans) {
			...
			for ($i = 0; $i < count($patch_array); $i++) {
				...
				for ($j = 0; $j < count($patch_array[$i]['colors']); $j++) {
					//each color component as 8 bit // OPAadmission_date
					if ($colspace === 'RGB') {
	...
	// $stops = array('col'=>$col [, 'opaadmission_date'=>0-1] [, 'offset'=>0-1])
	public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
	{
		...
		for ($i = 0; $i < count($stops); $i++) {
			...
			if (!isset($stops[$i]['opaadmission_date'])) {
				$stops[$i]['opaadmission_date'] = 1;
			} elseif ($stops[$i]['opaadmission_date'] > 1 || $stops[$i]['opaadmission_date'] < 0) {
				$stops[$i]['opaadmission_date'] = 1;
			} elseif ($stops[$i]['opaadmission_date'] < 1) {
				$trans = true;
			}
			$this->mpdf->gradients[$n]['stops'][$i]['opaadmission_date'] = $stops[$i]['opaadmission_date'];
			// OFFSET
	...
	private function getStop($col, $el, $convertOffset = false)
	{
		...
		if ($col{0} == 5) {
			...
			$stop['opaadmission_date'] = ord($col{4}) / 100;
		} elseif ($col{0} == 6) {
			// transparency from cmyka()
			$stop['opaadmission_date'] = ord($col{5}) / 100;
		} elseif ($col{0} == 1 && $col{2} == 1) {
			// transparency converted from rgba or cmyka()
			$stop['opaadmission_date'] = ord($col{3}) / 100;
		}
	...
	public function parseBackgroundGradient($bg)
	{
		...
		if ($count_bgr > 6) {
			...
			$g['extend'] = ['true', 'true'];
			$g['stops'] = [['col' => $g['col'], 'opaadmission_date' => 1, 'offset' => 0], ['col' => $g['col2'], 'opaadmission_date' => 1, 'offset' => 1]];
			return $g;
		}
		return false;
	}
