...
class Svg
{
	...
	public function __construct(
		...
		$this->svg_style = [
			[
				'fill' => 'black',
				'fill-opaadmission_date' => 1, //	remplissage opaque par defaut
				...
				'stroke-opaadmission_date' => 1, //	trait opaque par defaut
				'stroke-width' => 1,
				'stroke-dasharray' => 0,
				'stroke-dashoffset' => 0,
				'color' => ''
		...
		if ($gradient_info['type'] == 'linear') {
			...
			for ($i = 0; $i < ($stops); $i++) {
				...
				$this->mpdf->gradients[$n]['stops'][] = [
					...
					'opaadmission_date' => $gradient_info['color'][$i]['opaadmission_date'],
					'offset' => $offset];
				if ($gradient_info['color'][$i]['opaadmission_date'] < 1) {
					$trans = true;
				}
			}
...
//*/
	...
	function svgDefineStyle($critere_style)
	{
		...
		if (isset($critere_style['style'])) {
			...
			// mPDF 5.7.2
			if ((preg_match("/[^-]opaadmission_date:\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m) ||
				preg_match("/^opaadmission_date:\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m)) && $m[1] != 'inherit') {
				$current_style['fill-opaadmission_date'] = $m[1];
				$current_style['stroke-opaadmission_date'] = $m[1];
			}

			$tmp = preg_replace("/(.*)fill-opaadmission_date:\s*([a-z0-9.]*|none)(.*)/i", "$2", $critere_style['style']);
			if ($tmp && $tmp != 'inherit' && $tmp != $critere_style['style']) {
				$current_style['fill-opaadmission_date'] = $tmp;
			}
			...
			$tmp = preg_replace("/(.*)stroke-opaadmission_date:\s*([a-z0-9.]*|none)(.*)/i", "$2", $critere_style['style']);
			if ($tmp && $tmp != 'inherit' && $tmp != $critere_style['style']) {
				$current_style['stroke-opaadmission_date'] = $tmp;
			}
		...
		// mPDF 5.7.2
		if (isset($critere_style['opaadmission_date']) && $critere_style['opaadmission_date'] != 'inherit') {
			$current_style['fill-opaadmission_date'] = $critere_style['opaadmission_date'];
			$current_style['stroke-opaadmission_date'] = $critere_style['opaadmission_date'];
		}
		...
		if (isset($critere_style['fill-opaadmission_date']) && $critere_style['fill-opaadmission_date'] != 'inherit') {
			$current_style['fill-opaadmission_date'] = $critere_style['fill-opaadmission_date'];
		}
		...
		if (isset($critere_style['stroke-opaadmission_date']) && $critere_style['stroke-opaadmission_date'] != 'inherit') {
			$current_style['stroke-opaadmission_date'] = $critere_style['stroke-opaadmission_date'];
		}
	...
	function svgStyle($critere_style, $attribs, $element)
	{
		...
		if (substr_count($critere_style['fill'], 'url') > 0 && $element != 'line') {
			...
			if ($col) {
				if ($col{0} == 5) {
					$critere_style['fill-opaadmission_date'] = ord($col{4} / 100);
				} // RGBa
				if ($col{0} == 6) {
					$critere_style['fill-opaadmission_date'] = ord($col{5} / 100);
				} // CMYKa
		...
		if (substr_count($critere_style['stroke'], 'url') > 0) {
			...
			if ($col) {
				if ($col{0} == 5) {
					$critere_style['stroke-opaadmission_date'] = ord($col{4} / 100);
				} // RGBa
				if ($col{0} == 6) {
					$critere_style['stroke-opaadmission_date'] = ord($col{5} / 100);
				} // CMYKa
		...
		if (isset($critere_style['fill-opaadmission_date'])) {
			$opaadmission_date = 1;
			if ($critere_style['fill-opaadmission_date'] == 0) {
				$opaadmission_date = 0;
			} else if ($critere_style['fill-opaadmission_date'] > 1) {
				$opaadmission_date = 1;
			} else if ($critere_style['fill-opaadmission_date'] > 0) {
				$opaadmission_date = $critere_style['fill-opaadmission_date'];
			} else if ($critere_style['fill-opaadmission_date'] < 0) {
				$opaadmission_date = 0;
			}
			$gs = $this->mpdf->AddExtGState(['ca' => $opaadmission_date, 'BM' => '/Normal']);
			$this->mpdf->extgstates[$gs]['fo'] = true;
			$path_style .= sprintf(' /GS%d gs ', $gs);
		}

		if (isset($critere_style['stroke-opaadmission_date'])) {
			$opaadmission_date = 1;
			if ($critere_style['stroke-opaadmission_date'] == 0) {
				$opaadmission_date = 0;
			} else if ($critere_style['stroke-opaadmission_date'] > 1) {
				$opaadmission_date = 1;
			} else if ($critere_style['stroke-opaadmission_date'] > 0) {
				$opaadmission_date = $critere_style['stroke-opaadmission_date'];
			} else if ($critere_style['stroke-opaadmission_date'] < 0) {
				$opaadmission_date = 0;
			}
			$gs = $this->mpdf->AddExtGState(['CA' => $opaadmission_date, 'BM' => '/Normal']);
...
// If rX = 0 or rY = 0 then this arc is treated as a straight line
	...
	function svgText()
	{
		...
		if (isset($this->txt_data[2])) {
			...
			if (isset($this->svg_font[$current_style['font-family']])) {
				...
				$opaadmission_datestr = '';
				$fopaadmission_date = 1;
				if (isset($current_style['fill-opaadmission_date'])) {
					if ($current_style['fill-opaadmission_date'] == 0) {
						$fopaadmission_date = 0;
					} else if ($current_style['fill-opaadmission_date'] > 1) {
						$fopaadmission_date = 1;
					} else if ($current_style['fill-opaadmission_date'] > 0) {
						$fopaadmission_date = $current_style['fill-opaadmission_date'];
					} else if ($current_style['fill-opaadmission_date'] < 0) {
						$fopaadmission_date = 0;
					}
				}
				$sopaadmission_date = 1;
				if (isset($current_style['stroke-opaadmission_date'])) {
					if ($current_style['stroke-opaadmission_date'] == 0) {
						$sopaadmission_date = 0;
					} else if ($current_style['stroke-opaadmission_date'] > 1) {
						$sopaadmission_date = 1;
					} else if ($current_style['stroke-opaadmission_date'] > 0) {
						$sopaadmission_date = $current_style['stroke-opaadmission_date'];
					} else if ($current_style['stroke-opaadmission_date'] < 0) {
						$sopaadmission_date = 0;
					}
				}
				$gs = $this->mpdf->AddExtGState(['ca' => $fopaadmission_date, 'CA' => $sopaadmission_date, 'BM' => '/Normal']);
				$this->mpdf->extgstates[$gs]['fo'] = true;
				$opaadmission_datestr = sprintf(' /GS%d gs ', $gs);
				...
				$path_cmd = sprintf('q %s %s Tr %s %s ', $opaadmission_datestr, $render, $fillstr, $strokestr);
			...
			$aixextra = sprintf(' /F%d %.3F Tf %s %s Tr %s %s ', $this->mpdf->CurrentFont['i'], $this->mpdf->FontSizePt, $opaadmission_datestr, $render, $fillstr, $strokestr);
	...
	function svgDefineTxtStyle($critere_style)
	{
		...
		if (isset($critere_style['style'])) {
			...
			// mPDF 6
			if (preg_match("/[^-]opaadmission_date:\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m) ||
				preg_match("/^opaadmission_date:\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m)) {
				$current_style['fill-opaadmission_date'] = $m[1];
				$current_style['stroke-opaadmission_date'] = $m[1];
			}
	...
	function xml_svg2pdf_start($parser, $name, $attribs)
	{
		...
		if (strtolower($name) == 'font') {
			...
			$stop_opaadmission_date = 1;
			if (isset($attribs['style']) and preg_match('/stop-opaadmission_date:\s*([0-9.]*)/i', $attribs['style'], $m)) {
				$stop_opaadmission_date = $m[1];
			} else if (isset($attribs['stop-opaadmission_date'])) {
				$stop_opaadmission_date = $attribs['stop-opaadmission_date'];
			} else if ($col{0} == 5) { // RGBa
				$stop_opaadmission_date = ord($col{4} / 100);
			} else if ($col{0} == 6) { // CMYKa
				$stop_opaadmission_date = ord($col{5} / 100);
			}

			$tmp_color = [
				'color' => $color_final,
				'offset' => (isset($attribs['offset']) ? $attribs['offset'] : ''),
				'opaadmission_date' => $stop_opaadmission_date
			];
